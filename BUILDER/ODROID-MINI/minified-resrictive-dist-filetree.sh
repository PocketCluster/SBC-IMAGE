...
function ubuntu_essential() {
    # only the essentials
    chroot $R apt-get -y install --no-install-recommends --no-install-suggests language-pack-en-base ca-certificates isc-dhcp-client udev netbase ifupdown iproute iputils-ping net-tools ntpdate ntp tzdata dialog resolvconf
    ...
    # console & keyboard
    chroot $R apt-get -y install --no-install-recommends --no-install-suggests console-common console-data console-setup keyboard-configuration
    ...
    chroot $R apt-get -y install --no-install-recommends --no-install-suggests libpam-systemd dbus
}
...
function docker_setup() {
    # docker dependencies
    chroot $R apt-get -y install --no-install-recommends --no-install-suggests apparmor adduser iptables init-system-helpers lsb-base libapparmor1 libc6 libdevmapper1.02.1 
    # docker recommends
    chroot $R apt-get -y install --no-install-recommends --no-install-suggests cgroupfs-mount cgroup-lite git xz-utils
    # docker suggestion
    chroot $R apt-get -y install --no-install-recommends --no-install-suggests btrfs-tools
    # docker possible utility
    chroot $R apt-get -y install --no-install-recommends --no-install-suggests apparmor-profiles apparmor-utils bridge-utils

    # aufs is blocked for now as not in mainstream yet 4.9 maybe?
    # chroot $R apt-get -y install --no-install-recommends --no-install-suggests aufs-tools
