...
1 : start=     3072, size=   262144, Id= c, bootable
2 : start=   265216, size=  ${SIZE}, Id=83
3 : start=        0, size=        0, Id= 0
4 : start=        0, size=        0, Id= 0
EOM

    TOP_LOOP="$(losetup -o 0 --sizelimit $((3072 * 512)) -f --show ${BASEDIR}/${IMAGE})"
    BOOT_LOOP="$(losetup -o $((3072 * 512)) --sizelimit $((262144 * 512)) -f --show ${BASEDIR}/${IMAGE})"
    # ROOT_LOOP="$(losetup -o $((265216 * 512)) --sizelimit $((1433600 * 512)) -f --show ${BASEDIR}/${IMAGE})"
    ROOT_LOOP="$(losetup -o $((265216 * 512)) -f --show ${BASEDIR}/${IMAGE})"
    ...
    # Fusing to image
    local BL1="${PWD}"/bl1.bin.hardkernel
    local UBOOT="${PWD}"/u-boot.bin
    dd if="${BL1}"   of="${TOP_LOOP}" conv=fsync bs=1   count=442
    dd if="${BL1}"   of="${TOP_LOOP}" conv=fsync bs=512 skip=1     seek=1
    dd if="${UBOOT}" of="${TOP_LOOP}" conv=fsync bs=512 seek=97
    sync

    # Set u-boot
    chmod +x ./uboot-env
    ./uboot-env -d "${TOP_LOOP}" -o 0xB4000 -l 0x8000 del -I
    ./uboot-env -d "${TOP_LOOP}" -o 0xB4000 -l 0x8000 del -i
    cat ./bundle.uEnv | ./uboot-env -d "${TOP_LOOP}" -o 0xB4000 -l 0x8000 set
    sync

    # Copying image files
    #cp "${PWD}"/bl1.bin.hardkernel $R/boot/
    #cp "${PWD}"/u-boot.bin $R/boot/
    #cp "${PWD}"/sd_fusing.sh $R/boot/
    #cp "${PWD}"/boot.init $R/boot/
    ...
    rsync -a "$R/" "${MOUNTDIR}/"
    # U-571 part that should have been done in kernel patch stage
    #(cd ${MOUNTDIR}/boot/conf.d/; ln -s ./system.default ./default)
    #(cd ${MOUNTDIR}/boot/conf.d/system.default; ln -s ../../../media/boot/ ./kernel)
