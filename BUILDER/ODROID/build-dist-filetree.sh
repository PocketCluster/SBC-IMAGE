...
function setup_kernel() {
    ...
    chroot $R apt-get -q=2 -y install linux-image-c2 bootini
    # U-571
    mkdir -p $R/boot/conf.d/system.default
    cp ${PWD}/uEnv.txt > $R/boot/conf.d/system.default/
    (cd $R/boot/conf.d/; ln -s system.default default)
    (cd $R/boot/conf.d/system.default; ln -s $R/media/boot/ kernel)
}
