...
function bootstrap() {
    ...
    if [ ! -f "${R}/tmp/.bootstrap" ]; then
        if [ "${ARCH}" == "armv7l" ]; then
            ...
            qemu-debootstrap --variant=minbase --verbose --arch=armhf $RELEASE $R http://ports.ubuntu.com/
        fi
...
function configure_minimal_hardware() {
    ...
    # Linux Firmware has been blocked since it's usage is unclear
...
function configure_hardware() {
    ...
    # VideoCore libraries and binaries
    chroot $R apt-get -y install libraspberrypi-bin libraspberrypi-dev libraspberrypi-doc libraspberrypi0 
    # Firmware Kernel installation
    chroot $R apt-get -y install raspberrypi-bootloader rpi-update
    chroot $R apt-get -y install linux-firmware linux-firmware-nonfree
    chroot $R rpi-update
